# Documentation Index

| Document | Description |
|---|---|
| [NOVELTY.md](NOVELTY.md) | What makes this project different |
| [architecture/SYSTEM_ARCHITECTURE.md](architecture/SYSTEM_ARCHITECTURE.md) | Layers, data flow, tech choices |
| [agent/ARCHITECT_AGENT.md](agent/ARCHITECT_AGENT.md) | Proactive co-pilot design |
| [bci/ADAPTER_LAYER.md](bci/ADAPTER_LAYER.md) | Open tools → high-bandwidth path |
| [ux/UX_PRINCIPLES.md](ux/UX_PRINCIPLES.md) | BCI-native UX + wireframes |
| [ux/USER_GUIDE.md](ux/USER_GUIDE.md) | Everyday use for BCI users |
| [ux/DAILY_DRIVER.md](ux/DAILY_DRIVER.md) | Long sessions, multimodal, a11y |
| [ux/A11Y_AUDIT.md](ux/A11Y_AUDIT.md) | BCI accessibility release checklist |
| [assets/demo/](assets/demo/) | Companion UI demo GIF + stills |
| [STATUS.md](STATUS.md) | Feature map: what ships vs product brief |
| [ux/CAREGIVER_SETUP.md](ux/CAREGIVER_SETUP.md) | First-time install help |
| [bci/NEURALINK_READINESS.md](bci/NEURALINK_READINESS.md) | Future implant adapter path |
| [bci/BRAINFLOW.md](bci/BRAINFLOW.md) | Open EEG today |
| [architecture/FAILSAFE.md](architecture/FAILSAFE.md) | Override & degraded-signal safety |
| [privacy/THREAT_MODEL.md](privacy/THREAT_MODEL.md) | STRIDE-lite threat model |
| [CONTRIBUTING_ISSUES.md](CONTRIBUTING_ISSUES.md) | Good first issues |



| [architecture/LATENCY_BUDGET.md](architecture/LATENCY_BUDGET.md) | Latency budgets |
| [privacy/PRIVACY_ETHICS.md](privacy/PRIVACY_ETHICS.md) | Privacy, ethics, safety |
| [roadmap/ROADMAP.md](roadmap/ROADMAP.md) | Phased development |
| [roadmap/DAY1_PROTOTYPE.md](roadmap/DAY1_PROTOTYPE.md) | First working session plan |
| [research/OPEN_QUESTIONS.md](research/OPEN_QUESTIONS.md) | Risks and research next steps |

Start with the root [README](../README.md) and [TODO.md](../TODO.md).
