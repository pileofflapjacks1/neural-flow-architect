...
def test_recipe_suggestion() -> None:
    sug = recipe_suggestion(current_recipe="study", app_category="social", suggest_enabled=True)
    assert sug is not None
    assert sug["suggested_recipe"] == "social"
    assert (
        recipe_suggestion(current_recipe="study", app_category="study", suggest_enabled=True)
        is None
    )
